...
for ii = 1:length(data{1})
    ...
    if length(ev) == 1
        % Make time format easier to read later
        tt = strcat(extractBefore(ev(1).PreferredTime,' '), 'T',...
            extractAfter(ev(1).PreferredTime,' '));
        % #Network  Station  sLatitude  sLongitude  EventID  tOrigin  
        % eLatitude  eLongitude  Depth(km)
          fprintf(fid, '%-s %19s %19.3f %16.3f %17s %27s %25.3f %19.3f %20.2f \n', ...
              string(data{1}(ii)), string(data{2}(ii)), data{3}(ii), ...
              data{4}(ii), extractAfter(ev(1).PublicId, '='), ...
              tt, ev(1).PreferredLatitude, ...
        ...
        if length(ev) > maxlim
            for jj = 1:maxlim
                % Make time format easier to read later
                tt = strcat(extractBefore(evstruct(jj).PreferredTime,' '), 'T',...
                extractAfter(evstruct(jj).PreferredTime,' '));
                % #Network  Station  sLatitude  sLongitude  EventID  tOrigin  
                % eLatitude  eLongitude  Depth(km)
                fprintf(fid, '%-s %19s %19.3f %16.3f %17s %27s %25.3f %19.3f %20.2f \n', ...
                    string(data{1}(ii)), string(data{2}(ii)), data{3}(ii), ...
                    data{4}(ii), extractAfter(evstruct(jj).PublicId, '='), ...
                    tt, evstruct(jj).PreferredLatitude, ...
